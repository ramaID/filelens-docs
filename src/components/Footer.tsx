    ...
    <div className="flex">
      {previousPage && (
        <div className="flex flex-col items-start gap-3">
          <PageLink label="Sebelumnya" page={previousPage} previous />
        </div>
      )}
      {nextPage && (
        <div className="ml-auto flex flex-col items-end gap-3">
          <PageLink label="Selanjutnya" page={nextPage} />
        </div>
      )}
    </div>
  )
  ...
  return (
    <div className="flex flex-col items-center justify-between gap-5 border-t border-zinc-900/5 pt-8 sm:flex-row dark:border-white/5">
      <p className="text-xs text-zinc-600 dark:text-zinc-400">
        &copy; Hak Cipta {new Date().getFullYear()}. Seluruh hak dilindungi.
      </p>
      <div className="flex gap-4">
        <SocialLink href="https://x.com/ramageek" icon={XIcon}>
          Follow us on X
        </SocialLink>
        <SocialLink href="https://github.com/ramaID" icon={GitHubIcon}>
          Follow us on GitHub
        </SocialLink>
      </div>
    </div>
